...
chaves = pd.read_json("data/raw/chaves_apto.json")

# Limpeza do dados da Chaves na Mão
## Processos iniciais
chaves.drop(index=[1402, 1661, 2270], inplace=True)
...
chaves['quartos'] = chaves['quartos'].astype(str).str.replace('[^0-9]', '', regex=True)
...
chaves = chaves[chaves['quartos']<=10]
chaves.reset_index(drop=True, inplace=True)

## Salvando os dados em .csv 
chaves.to_csv("data/processed/chaves_aptos.csv", index=False)
